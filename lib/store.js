export class Store {
  ...
  /**
   * setState works like a simplified version of lodash's _.set(), only it sets this.state instead of an arbitrary object.
   * Usage example:
   *   setState('city.street[0].color', 'brown');
   */
  setState(propertyString, value) {
    ...
    // Publish a generic "state was updated" event, and an event for the specific property updated.
    this.publish('state', this.state);
    this.publish(propertyString, value);
  }
